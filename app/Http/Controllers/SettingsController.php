...
class SettingsController extends Controller
{
    public function index()
    {
        ...
        if ($isAdmin) {
            ...
            $props['documentSettings'] = [
                'pdf_primary_color'    => Setting::get('pdf_primary_color', '#1a56db'),
                'pdf_footer_text'      => Setting::get('pdf_footer_text', ''),
                'pdf_bank_name'        => Setting::get('pdf_bank_name', ''),
                'pdf_iban'             => Setting::get('pdf_iban', ''),
                'pdf_bic'              => Setting::get('pdf_bic', ''),
                'pdf_show_bank_details'=> Setting::get('pdf_show_bank_details', '0'),
                'pdf_invoice_intro'    => Setting::get('pdf_invoice_intro', ''),
                'pdf_quote_intro'      => Setting::get('pdf_quote_intro', ''),
            ];
    ...
    public function saveDocumentDesign(Request $request)
    {
        $this->ensureAdmin();

        $request->validate([
            'pdf_primary_color'     => 'required|string|max:7',
            'pdf_footer_text'       => 'nullable|string|max:500',
            'pdf_bank_name'         => 'nullable|string|max:255',
            'pdf_iban'              => 'nullable|string|max:50',
            'pdf_bic'               => 'nullable|string|max:20',
            'pdf_show_bank_details' => 'nullable|boolean',
            'pdf_invoice_intro'     => 'nullable|string|max:1000',
            'pdf_quote_intro'       => 'nullable|string|max:1000',
        ]);

        $fields = ['pdf_primary_color', 'pdf_footer_text', 'pdf_bank_name', 'pdf_iban', 'pdf_bic', 'pdf_invoice_intro', 'pdf_quote_intro'];
        foreach ($fields as $field) {
            Setting::set($field, $request->input($field, ''));
        }
        Setting::set('pdf_show_bank_details', $request->boolean('pdf_show_bank_details') ? '1' : '0');

        return redirect()->back()->with('success', 'Dokumenten-Design gespeichert.');
    }
    ...
    public function testEmail(Request $request)
    {
        ...
        try {
            ...
            // Laravel 11 (Symfony Mailer): scheme values are "smtp", "smtps", "tls"
            $encryption = Setting::get('mail_encryption', 'tls');
            // Symfony Mailer only supports "smtp" and "smtps" as schemes.
            // "smtps" = SSL-wrapped (port 465)
            // "smtp"  = plain or STARTTLS auto-negotiated (port 587)
            ...
            $recipient = $request->filled('recipient')
                ? $request->input('recipient')
                : auth()->user()->email;

            Mail::mailer('smtp')->raw('Dies ist eine Test-E-Mail von ' . config('app.name') . '. Wenn Sie diese Nachricht erhalten, funktioniert die E-Mail-Konfiguration korrekt.', function ($message) use ($recipient) {
                $message->to($recipient)
                    ->subject('Test-E-Mail - ' . config('app.name'));
            });

            return response()->json(['success' => true, 'message' => 'Test-E-Mail wurde an ' . $recipient . ' gesendet.']);
        } catch (\Exception $e) {
    ...
    {
        $key = 'hlsdk_' . \Illuminate\Support\Str::random(40);
        return response()->json([
            'success' => true,
            'key' => $key,
            'message' => 'API-Schlüssel erstellt. Bitte notieren Sie ihn, er wird nicht erneut angezeigt.',
        ]);
    }
