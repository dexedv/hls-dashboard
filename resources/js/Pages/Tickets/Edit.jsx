...
export default function TicketEdit() {
    const { ticket, customers, projects, users } = usePage().props;
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        assigned_users: ticket.assignees?.map(a => a.id) || [],
    ...
    return (
        <DashboardLayout title="Ticket bearbeiten">
            <Head title={`Ticket #${ticket.id} bearbeiten`} />

            <nav className="flex items-center gap-2 text-sm text-gray-500 mb-6">
                <Link href="/tickets" className="hover:text-primary-600">Tickets</Link>
                <span>/</span>
                <Link href={route('tickets.show', ticket.id)} className="hover:text-primary-600">#{ticket.id} {ticket.title}</Link>
                <span>/</span>
            ...
            <form onSubmit={handleSubmit} className="max-w-4xl">
                <div className="mb-6 flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900">Ticket bearbeiten</h1>
