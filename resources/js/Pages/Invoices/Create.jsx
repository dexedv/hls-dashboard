...
export default function Create() {
    ...
    return (
        <DashboardLayout title="Neue Rechnung">
            ...
            <form onSubmit={handleSubmit} className="max-w-6xl">
                <div className="mb-6 flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 mt-1">Erstellen Sie eine neue Rechnung</p>
                    </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <select value={data.currency} onChange={e => setData('currency', e.target.value)}
                                    className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-primary-500">
                                    {['EUR','USD','GBP','CHF','JPY','CAD'].map(c => <option key={c} value={c}>{c}</option>)}
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea value={data.notes} onChange={(e) => setData('notes', e.target.value)}
                                    rows={3} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-primary-500" />
                            </div>
                            ...
                            <div className="border-t border-gray-100 pt-4">
                                <label className="flex items-center gap-2 cursor-pointer mb-3">
                                    ...
                                    <span className="text-sm font-medium text-gray-700">Wiederkehrende Rechnung</span>
                                </label>
                                {data.is_recurring && (
                                    <div className="space-y-3 pl-5">
                                        <div>
                                            <label className="block text-xs text-gray-500 mb-1">Intervall</label>
                                            <select value={data.recurring_interval} onChange={e => setData('recurring_interval', e.target.value)}
                                        ...
                                        <div>
                                            <label className="block text-xs text-gray-500 mb-1">Nächste Ausführung</label>
                                            <input type="date" value={data.recurring_next_date} onChange={e => setData('recurring_next_date', e.target.value)}
                                                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" />
                                        </div>
                                        <div>
                                            <label className="block text-xs text-gray-500 mb-1">Enddatum (optional)</label>
                                            <input type="date" value={data.recurring_end_date} onChange={e => setData('recurring_end_date', e.target.value)}
                                                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" />
                                        </div>
                                    </div>
                                )}
