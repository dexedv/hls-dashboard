...
const allTabs = [
    ...
    { id: 'documents', label: 'Dokumente', adminOnly: true, icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' },
...
export default function SettingsIndex({ settings, emailSettings, documentSettings, licenseInfo, userCount, twoFactorEnabled, isAdmin }) {
        ...
        <DashboardLayout title="Einstellungen">
            ...
            <div className="flex flex-col lg:flex-row gap-6">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {activeTab === 'documents' && <DocumentDesignTab documentSettings={documentSettings} />}
...
function EmailTab({ emailSettings }) {
    ...
    const testEmail = async () => {
        ...
        setTestResult(null);
        try {
            const res = await axios.post(route('settings.testEmail'), { recipient: testRecipient || undefined });
            ...
            <div className="space-y-4">
                ...
                <div className="flex flex-col sm:flex-row gap-2 justify-end">
                    <input
                        ...
                        placeholder="Empfänger (leer = eigene Adresse)"
                        className="flex-1 border border-gray-200 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary-500"
                    />
                    <button onClick={testEmail} disabled={testing || !form.mail_host}
                        className="px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white text-sm font-medium rounded-lg transition-colors disabled:opacity-50 whitespace-nowrap">
...
function DocumentDesignTab({ documentSettings }) {
    const [form, setForm] = useState({
        pdf_primary_color:     documentSettings?.pdf_primary_color    || '#1a56db',
        pdf_footer_text:       documentSettings?.pdf_footer_text      || '',
        pdf_bank_name:         documentSettings?.pdf_bank_name        || '',
        pdf_iban:              documentSettings?.pdf_iban             || '',
        pdf_bic:               documentSettings?.pdf_bic              || '',
        pdf_show_bank_details: documentSettings?.pdf_show_bank_details === '1',
        pdf_invoice_intro:     documentSettings?.pdf_invoice_intro    || '',
        pdf_quote_intro:       documentSettings?.pdf_quote_intro      || '',
    });
    const [saving, setSaving] = useState(false);
    const update = (field, value) => setForm(prev => ({ ...prev, [field]: value }));
    const save = () => {
        setSaving(true);
        router.post(route('settings.saveDocumentDesign'), form, { onFinish: () => setSaving(false) });
    };

    return (
        <div className="space-y-6">
            <Card title="Farbe & Erscheinungsbild" description="Anpassung der Farben für Rechnungen und Angebote">
                <div className="space-y-4">
                    <div className="flex items-center gap-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Hauptfarbe</label>
                            <div className="flex items-center gap-3">
                                <input
                                    type="color"
                                    value={form.pdf_primary_color}
                                    onChange={e => update('pdf_primary_color', e.target.value)}
                                    className="h-10 w-16 cursor-pointer rounded border border-gray-200 dark:border-gray-600 p-0.5 bg-white dark:bg-gray-700"
                                />
                                <input
                                    type="text"
                                    value={form.pdf_primary_color}
                                    onChange={e => update('pdf_primary_color', e.target.value)}
                                    placeholder="#1a56db"
                                    className="w-32 border border-gray-200 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary-500"
                                />
                                <div className="w-8 h-8 rounded" style={{ backgroundColor: form.pdf_primary_color }} />
                            </div>
                            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Wird für Überschriften und Tabellenkopf verwendet</p>
                        </div>
                    </div>
                    <div className="flex justify-end"><SaveButton saving={saving} onClick={save} /></div>
                </div>
            ...
            <Card title="Einleitungstext" description="Optionaler Text unterhalb des Titels, vor der Positionstabelle">
                <div className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Einleitungstext Rechnungen</label>
                        <textarea
                            value={form.pdf_invoice_intro}
                            onChange={e => update('pdf_invoice_intro', e.target.value)}
                            rows={3}
                            placeholder="z.B. Vielen Dank für Ihren Auftrag. Wir erlauben uns, folgende Positionen in Rechnung zu stellen:"
                            className="w-full border border-gray-200 dark:border-gray-600 rounded-lg px-4 py-2.5 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary-500 resize-none"
                        />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Einleitungstext Angebote</label>
                        <textarea
                            value={form.pdf_quote_intro}
                            onChange={e => update('pdf_quote_intro', e.target.value)}
                            rows={3}
                            placeholder="z.B. Vielen Dank für Ihre Anfrage. Gerne unterbreiten wir Ihnen folgendes Angebot:"
                            className="w-full border border-gray-200 dark:border-gray-600 rounded-lg px-4 py-2.5 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary-500 resize-none"
                        />
                    </div>
                    <div className="flex justify-end"><SaveButton saving={saving} onClick={save} /></div>
            ...
            <Card title="Bankverbindung" description="Wird am Ende von Rechnungen und Angeboten angezeigt">
                <div className="space-y-4">
                    <label className="flex items-center gap-3 cursor-pointer">
                        <input
                            type="checkbox"
                            checked={form.pdf_show_bank_details}
                            onChange={e => update('pdf_show_bank_details', e.target.checked)}
                            className="w-4 h-4 text-primary-600 rounded border-gray-300 focus:ring-primary-500"
                        />
                        <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Bankverbindung auf Dokumenten anzeigen</span>
                    </label>
                    {form.pdf_show_bank_details && (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                            <InputField label="Bank" type="text" placeholder="Musterbank AG" value={form.pdf_bank_name} onChange={e => update('pdf_bank_name', e.target.value)} />
                            <InputField label="IBAN" type="text" placeholder="DE12 3456 7890 1234 5678 90" value={form.pdf_iban} onChange={e => update('pdf_iban', e.target.value)} />
                            <InputField label="BIC" type="text" placeholder="MUSTDE12XXX" value={form.pdf_bic} onChange={e => update('pdf_bic', e.target.value)} />
                        </div>
                    )}
                    <div className="flex justify-end"><SaveButton saving={saving} onClick={save} /></div>
                </div>
            ...
            <Card title="Fußzeile" description="Eigener Text für die Fußzeile aller PDF-Dokumente">
                <div className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Fußzeilen-Text</label>
                        <textarea
                            value={form.pdf_footer_text}
                            onChange={e => update('pdf_footer_text', e.target.value)}
                            rows={2}
                            placeholder="Leer lassen für automatischen Footer (Firmenname + Adresse)"
                            className="w-full border border-gray-200 dark:border-gray-600 rounded-lg px-4 py-2.5 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary-500 resize-none"
                        />
                        <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Leer lassen → Firmenname und Adresse werden automatisch verwendet</p>
                    </div>
                    <div className="flex justify-end"><SaveButton saving={saving} onClick={save} /></div>
                </div>
            </Card>
