...
export default function IntegrationsIndex() {
    ...
    const testConnection = async () => {
        ...
        try {
            const response = await axios.get('/lexware/test');
            setConnectionStatus(response.data);
        } catch (error) {
            setConnectionStatus({ success: false, message: error.response?.data?.message || error.message });
        }
    ...
    const handleSaveApiKey = async () => {
        try {
            const response = await axios.post('/lexware/save-api-key', apiKeyData);
            setConnectionStatus(response.data);
    ...
    const syncAllCustomers = async () => {
        try {
            const response = await axios.post('/lexware/customers/sync-all');
    ...
    const syncAllInvoices = async () => {
        try {
            const response = await axios.post('/lexware/invoices/sync-all');
            const result = response.data;
    ...
    const syncAllQuotes = async () => {
        try {
            const response = await axios.post('/lexware/quotes/sync-all');
            const result = response.data;
    ...
    const importCustomers = async () => {
        try {
            const response = await axios.post('/lexware/customers/import');
            const result = response.data;
    ...
    const importInvoices = async () => {
        try {
            const response = await axios.post('/lexware/invoices/import');
            const result = response.data;
    ...
    const handleCreateApiKey = async () => {
        ...
        try {
            const response = await axios.post(route('integrations.createApiKey'));
            setGeneratedApiKey(response.data.key);
        } catch (err) {
            alert(err.response?.data?.error || 'Fehler beim Erstellen des API-Schlüssels');
        } finally {
                ...
                {canManage ? (
                    <>
                        <Button onClick={handleCreateApiKey} disabled={creatingApiKey}>
                            {creatingApiKey ? 'Wird erstellt...' : 'Neuen API-Schluessel erstellen'}
                        </Button>
                        {generatedApiKey && (
                            <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                                <p className="text-sm font-medium text-yellow-800 mb-2">
                                    API-Schlüssel erstellt — bitte jetzt kopieren, er wird nicht erneut angezeigt:
                                </p>
                                <div className="flex items-center gap-2">
                                    <code className="flex-1 text-xs bg-white border border-yellow-300 rounded px-3 py-2 font-mono break-all">
                                        {generatedApiKey}
                                    </code>
                                    <button
                                        onClick={() => { navigator.clipboard.writeText(generatedApiKey); }}
                                        className="px-3 py-2 text-xs bg-yellow-100 border border-yellow-300 rounded hover:bg-yellow-200 transition-colors shrink-0"
                                    >
                                        Kopieren
                                    </button>
                                </div>
