...
<head>
    ...
    @php
        ...
        $primaryColor  = \App\Models\Setting::get('pdf_primary_color', '#1a56db');
        $footerText    = \App\Models\Setting::get('pdf_footer_text', '');
        $showBank      = \App\Models\Setting::get('pdf_show_bank_details', '0') === '1';
        $bankName      = \App\Models\Setting::get('pdf_bank_name', '');
        $iban          = \App\Models\Setting::get('pdf_iban', '');
        $bic           = \App\Models\Setting::get('pdf_bic', '');
        $introText     = \App\Models\Setting::get('pdf_invoice_intro', '');
    @endphp
    <style>
        ...
        .invoice-title { font-size: 28px; font-weight: bold; color: {{ $primaryColor }}; margin-bottom: 20px; }
        .meta-table { width: 100%; margin-bottom: 30px; }
        .meta-table td { padding: 4px 0; }
        .meta-label { font-weight: bold; width: 150px; }
        .intro-text { margin-bottom: 20px; font-size: 12px; color: #444; }
        .items-table { width: 100%; border-collapse: collapse; margin-bottom: 20px; }
        .items-table th { background: {{ $primaryColor }}; color: white; padding: 10px 8px; text-align: left; font-size: 11px; }
        ...
        .totals .total-row { font-weight: bold; font-size: 14px; border-top: 2px solid {{ $primaryColor }}; }
        .footer { margin-top: 60px; padding-top: 20px; border-top: 1px solid #e5e7eb; font-size: 10px; color: #6b7280; text-align: center; }
        .bank-details { margin-top: 30px; padding: 12px; background: #f9fafb; border: 1px solid #e5e7eb; font-size: 11px; }
        .bank-details strong { display: block; margin-bottom: 4px; }
...
<body>
    ...
    <table class="meta-table">
        ...
        <tr>
            <td class="meta-label">Faelligkeitsdatum:</td>
    ...
    <table class="totals">
        <tr>
            <td>Nettobetrag:</td>
            <td class="text-right">{{ number_format($invoice->subtotal ?? 0, 2, ',', '.') }} {{ $currency }}</td>
        </tr>
        <tr>
            <td>MwSt. ({{ $taxRate }}%):</td>
            <td class="text-right">{{ number_format($invoice->tax ?? 0, 2, ',', '.') }} {{ $currency }}</td>
        </tr>
        <tr class="total-row">
            <td>Gesamtbetrag:</td>
            <td class="text-right">{{ number_format($invoice->total ?? 0, 2, ',', '.') }} {{ $currency }}</td>
        </tr>
    </table>
    ...
    @if($showBank && ($bankName || $iban || $bic))
    <div class="bank-details">
        <strong>Bankverbindung</strong>
        {{ implode(' | ', array_filter([$bankName, $iban ? 'IBAN: ' . $iban : '', $bic ? 'BIC: ' . $bic : ''])) }}
    </div>
    @endif
